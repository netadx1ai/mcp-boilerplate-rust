...
async fn test_blog_content_generation_workflow() {
    let result = timeout(
        ...
        test_blog_generation_scenarios()
    ).await;
    
    assert!(result.is_ok(), "Blog generation workflow test should not timeout");
    result.unwrap().expect("Server should generate blog content correctly");
}
...
async fn test_server_compilation_and_cli() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(help_text.contains("blog-generation-server"), "Help should contain program name");
...
async fn test_server_cli_argument_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let invalid_transport = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--transport", "invalid"])
        .current_dir("examples/blog-generation-server")
        .output()?;
    ...
    let invalid_port = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--port", "-1"])
        .current_dir("examples/blog-generation-server")
        .output()?;
    ...
    let invalid_delay = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--delay", "-1"])
        .current_dir("examples/blog-generation-server")
        .output()?;
    ...
    let valid_args = Command::new("cargo")
        .args(&[
            "run", "--bin", "blog-generation-server", "--", 
            "--transport", "http", 
            "--port", "3012", 
            "--host", "127.0.0.1",
            "--delay", "1",
...
async fn test_server_transport_configurations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stdio_process = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--transport", "stdio"])
        .current_dir("examples/blog-generation-server")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn();
    ...
    let http_process = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--transport", "http"])
        .current_dir("examples/blog-generation-server")
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn();
...
async fn test_server_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let unknown_flag = Command::new("cargo")
        .args(&["run", "--bin", "blog-generation-server", "--", "--unknown-flag"])
        .current_dir("examples/blog-generation-server")
        .output()?;
