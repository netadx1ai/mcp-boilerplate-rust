...
async fn test_filesystem_server_stdio_transport() {
    ...
    result
        .unwrap()
        .expect("Filesystem server should support STDIO transport");
}
...
async fn test_filesystem_server_http_transport() {
    ...
    result
        .unwrap()
        .expect("Filesystem server should support HTTP transport");
}
...
async fn test_all_servers_transport_modes() {
    ...
    for server_name in &servers {
        ...
        assert!(
            stdio_result.is_ok(),
            "STDIO transport should not timeout for {}",
            server_name
        );
        stdio_result
            .unwrap()
            .expect(&format!("{} should support STDIO transport", server_name));
        ...
        assert!(
            http_result.is_ok(),
            "HTTP transport should not timeout for {}",
            server_name
        );
        http_result
            .unwrap()
            .expect(&format!("{} should support HTTP transport", server_name));
    }
}
...
async fn test_transport_error_handling() {
    ...
    result
        .unwrap()
        .expect("Server should handle invalid transport modes gracefully");
}
...
async fn test_server_lifecycle_stdio() {
    ...
    result
        .unwrap()
        .expect("Server should start and stop cleanly with STDIO");
}

/// Helper function to test STDIO transport mode
async fn test_server_stdio_mode(
    server_name: &str,
    ...
    match child.try_wait()? {
        ...
        Some(status) => {
            ...
            if status.success() {
                ...
                println!(
                    "⚠️ {} STDIO transport exited with status: {}",
                    server_name, status
                );
...
async fn test_server_http_mode(
    server_name: &str,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let temp_dir = TempDir::new()?;
    ...
    match child.try_wait()? {
        ...
        Some(status) => {
            ...
            if status.success() {
                ...
                println!(
                    "⚠️ {} HTTP transport exited with status: {}",
                    server_name, status
                );
            }
...
async fn test_server_transport_help(
    server_name: &str,
    transport: &str,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    {
        println!(
            "✅ {} supports {} transport (mentioned in help)",
            server_name, transport
        );
    } else {
        // Even if not mentioned, server exists and responds to help
        println!(
            "✅ {} {} transport test passed (basic functionality)",
            server_name, transport
        );
    }
...
async fn test_invalid_transport_mode(
    server_name: &str,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let output = Command::new("cargo")
        .args(&[
    ...
    if output.status.success() {
        return Err(format!(
            "Server {} should reject invalid transport modes",
            server_name
        )
        .into());
    }
    ...
    if !stderr.is_empty() || !stdout.is_empty() {
        ...
        println!(
            "⚠️ {} should provide error message for invalid transport",
            server_name
        );
    }
    ...
    for cycle in 1..=3 {
        ...
        match child.try_wait()? {
            ...
            Some(status) => {
                ...
                println!(
                    "  Cycle {} completed (server exited with {})",
                    cycle, status
                );
            }
        }
...
async fn test_transport_performance() {
    ...
    for server_name in &servers {
        ...
        let result = timeout(Duration::from_secs(8), test_server_stdio_mode(server_name)).await;

        let elapsed = start_time.elapsed();

        assert!(
            result.is_ok(),
            "Transport performance test should not timeout for {}",
            server_name
        );
        result
            .unwrap()
            .expect(&format!("{} transport should work", server_name));

        // Performance target: transport startup should be < 3 seconds
        assert!(
            elapsed < Duration::from_secs(3),
            "Transport startup for {} took {:?}, should be < 3s",
            server_name,
            elapsed
        );
...
async fn test_concurrent_transports() {
    let result = timeout(Duration::from_secs(15), test_multiple_server_instances()).await;

    assert!(
        result.is_ok(),
        "Concurrent transport test should not timeout"
    );
    result
        .unwrap()
        .expect("Multiple server instances should work concurrently");
}
...
async fn test_multiple_server_instances() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for server_name in &servers {
        ...
        let handle =
            tokio::spawn(async move { test_server_transport_help(server_name, "stdio").await });
        handles.push(handle);
    }
