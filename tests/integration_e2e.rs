        ...
        for (name, handle) in servers.iter() {
            if let Some(port) = handle.port {
                if used_ports.contains(&port) {
                    eprintln!(
                        "❌ Port conflict detected: {} trying to use port {}",
                        name, port
                    );
                    conflicts = true;
                } else {
...
async fn test_multiple_servers_no_conflicts() {
    let test_result = timeout(Duration::from_secs(15), async {
        ...
        assert!(
            no_conflicts,
            "No port conflicts should exist between servers"
        );
        ...
        assert_eq!(
            still_running, 0,
            "All servers should be stopped after shutdown"
        );
    ...
    let test_result = timeout(Duration::from_secs(20), async {
        ...
        for i in 0..5 {
            ...
            let task = tokio::spawn(async move {
                ...
                println!(
                    "🔄 Concurrent check #{}: {} servers running",
                    i + 1,
                    running_count
                );
            });
        ...
        println!(
            "📈 Concurrent operations: min={}, max={} servers running",
...
async fn test_http_port_allocation() {
    let test_result = timeout(Duration::from_secs(10), async {
        ...
        assert!(
            accessible_ports >= 1,
            "At least one HTTP server should be accessible"
        );
...
async fn test_graceful_multi_server_shutdown() {
    let test_result = timeout(Duration::from_secs(12), async {
        ...
        assert!(
            running_count >= 2,
            "Multiple servers should be running before shutdown test"
        );
        ...
        assert_eq!(
            still_running, 0,
            "All servers should be stopped after graceful shutdown"
        );
            ...
            match ServerHandle::start(server_config).await {
                Ok(mut handle) => {
                    ...
                    println!(
                        ...
                        server_config.name, startup_time
                    );
...
async fn test_mixed_transport_modes() {
    let test_result = timeout(Duration::from_secs(12), async {
        ...
        println!(
            "📊 Mixed transport servers: {}/{} running",
            running_count,
            status.len()
        );
        ...
        assert!(
            no_conflicts,
            "HTTP servers in mixed mode should not have port conflicts"
        );

        // At least some servers should be running
        assert!(
            running_count >= 2,
            "At least 2 servers should run in mixed transport mode"
        );
...
async fn count_rust_processes() -> usize {
    match Command::new("pgrep").args(&["-f", "cargo|rust"]).output() {
        Ok(output) => {
...
async fn test_server_startup_error_handling() {
    let test_result = timeout(Duration::from_secs(8), async {
        ...
        println!(
            "📊 Servers running with conflicts: {}/{}",
            running_count,
            status.len()
        );
...
async fn test_rapid_server_cycles() {
    let test_result = timeout(Duration::from_secs(20), async {
        ...
        println!(
            "✅ Rapid cycling test: {}/3 cycles successful",
            successful_cycles
        );
        Ok::<(), Box<dyn std::error::Error>>(())
    })
