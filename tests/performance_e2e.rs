...
impl PerformanceMetrics {
    ...
    fn max_response_time(&self) -> Duration {
        self.response_times.iter().max().copied().unwrap_or(Duration::from_secs(0))
    }
...
impl ServerPerformanceTester {
    ...
    async fn test_startup_performance(&self, server_name: &str, binary_name: &str) -> Result<Duration, Box<dyn std::error::Error>> {
        ...
        let result = timeout(
            ...
            async {
                ...
                if !output.status.success() {
                    return Err(format!("Server {} failed to start: stderr={}", 
                                     server_name, String::from_utf8_lossy(&output.stderr)).into());
                }
        ...
        // Update metrics
        {
            ...
            let server_metrics = metrics.entry(server_name.to_string()).or_insert_with(PerformanceMetrics::new);
    ...
    async fn test_memory_usage(&self, server_name: &str, binary_name: &str) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    async fn test_load_performance(&self, server_name: &str, binary_name: &str) -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    let task = tokio::spawn(async move {
                        ...
                        match output {
                            Ok(output) if output.status.success() => {
                                println!("✅ Request #{} for {} completed in {:?}", i + 1, server_name_copy, response_time);
                                Ok(response_time)
                            }
                            Ok(output) => {
                                eprintln!("⚠️ Request #{} for {} failed with status: {}", i + 1, server_name_copy, output.status);
                                Err("Request failed".into())
                            }
                            Err(e) => {
                                eprintln!("❌ Request #{} for {} error: {}", i + 1, server_name_copy, e);
                                Err(e.into())
                            }
                        }
                    });
    ...
    async fn generate_report(&self) -> String {
        ...
        for (server_name, server_metrics) in metrics.iter() {
            ...
            report.push_str(&format!("   Max Response: {:?}\n", server_metrics.max_response_time()));
...
async fn test_server_startup_times() {
    ...
    for (server_name, binary_name) in servers {
        match tester.test_startup_performance(server_name, binary_name).await {
            Ok(startup_time) => {
    ...
    let avg_startup: Duration = startup_times.iter()
        .map(|(_, time)| *time)
        .sum::<Duration>() / startup_times.len() as u32;
    ...
    match test_result {
        Ok(Ok(())) => {
            ...
            if let Some(server_metrics) = metrics.get(server_name) {
                ...
                println!("   Average response: {:?}", server_metrics.average_response_time());
                println!("   Max response: {:?}", server_metrics.max_response_time());
                println!("   Success rate: {:.1}%", server_metrics.success_rate() * 100.0);
...
async fn test_large_payload_handling() {
    let test_result = timeout(
        ...
        async {
            ...
            println!("✅ Large payload handling test: {}/{} cases successful", successful_cases, test_cases.len());
            Ok::<(), Box<dyn std::error::Error>>(())
        }
    ).await;
...
async fn test_timeout_handling() {
    let test_result = timeout(
        ...
        async {
                    ...
                    async {
                        ...
                        match output {
                            ...
                            Ok(output) => {
                                ...
                                if stdout.contains("help") || stdout.contains("version") ||
                                   stderr.contains("help") || stderr.contains("version") {
                                    println!("✅ Operation '{}' provided expected output", op_name);
                                    Ok(())
                                } else {
                                    Err(format!("Operation '{}' failed: status={}", op_name, output.status).into())
                                }
                            }
            ...
            println!("✅ Timeout handling test: {}/{} operations successful", successful_ops, operations.len());
            Ok::<(), Box<dyn std::error::Error>>(())
        }
    ).await;
        ...
        async {
            ...
            for (server_name, binary_name) in servers {
                ...
                let stress_tests = vec![
                    ("invalid-port", vec!["--transport", "http", "--port", "99999"]),
                    ("invalid-transport", vec!["--transport", "invalid-mode"]),
                    ("conflicting-args", vec!["--transport", "http", "--transport", "stdio"]),
                ];
        ...
        async {
            ...
            for (scenario_name, error_args, recovery_args) in error_scenarios {
                ...
                match error_output {
                        ...
                        match recovery_output {
                            ...
                            Ok(output) => {
                                ...
                                if stdout.contains("help") || stdout.contains("version") ||
                                   stderr.contains("help") || stderr.contains("version") {
                                    recovery_successes += 1;
                                    println!("✅ Recovery provided expected output for '{}'", scenario_name);
                                } else {
                    ...
                    Err(e) => {
                        eprintln!("❌ Failed to test error condition for '{}': {}", scenario_name, e);
                    }
                }
            ...
            println!("✅ Error recovery test: {}/{} scenarios successful", recovery_successes, error_scenarios.len());
            Ok::<(), Box<dyn std::error::Error>>(())
        }
    ).await;
    ...
    let test_result = timeout(
        ...
        async {
            ...
            for (server_name, binary_name) in servers {
                ...
                // Startup performance
                if let Ok(startup_time) = tester.test_startup_performance(server_name, binary_name).await {
                    assert!(startup_time < Duration::from_secs(8), "Startup time requirement");
                }
...
async fn test_server_resilience_cycles() {
    let test_result = timeout(
        ...
        async {
                    ...
                    Ok(mut process) => {
                        ...
                        match process.kill() {
                            Ok(()) => {
                                let _ = process.wait();
                                successful_cycles += 1;
                                println!("✅ Cycle #{} completed in {:?}", cycle, cycle_start.elapsed());
                            }
                            Err(e) => {
            ...
            println!("✅ Server resilience: {}/{} cycles successful", successful_cycles, total_cycles);
            Ok::<(), Box<dyn std::error::Error>>(())
        }
    ).await;
...
async fn test_system_resource_limits() {
    let test_result = timeout(
        ...
        async {
            ...
            match tester.test_load_performance("filesystem", "filesystem-server").await {
                Ok(()) => {
                    let metrics = tester.metrics.lock().await;
                    if let Some(fs_metrics) = metrics.get("filesystem") {
                        ...
                        println!("   Average response: {:?}", fs_metrics.average_response_time());
    ...
    let test_result = timeout(
        ...
        async {
            ...
            if !baseline_times.is_empty() {
                let avg_baseline = baseline_times.iter().sum::<Duration>() / baseline_times.len() as u32;
                ...
                println!("💾 Baseline established: {:?} ±{:?}", avg_baseline, consistency);
            }
