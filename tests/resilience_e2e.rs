...
impl ServerResilienceTester {
    ...
    async fn test_invalid_tool_call_recovery(
        &self,
        server_name: &str,
        binary_name: &str,
        ...
        let result = timeout(self.config.recovery_timeout, async {
                ...
                match invalid_output {
                    Ok(output) => {
                        ...
                        if !output.status.success() {
                            ...
                            {
                                println!(
                                    "✅ Pattern #{} failed gracefully with helpful output",
                                    i + 1
                                );
                            } else {
        ...
        println!(
            "✅ Invalid tool call recovery test completed in {:?}",
            recovery_time
        );
        Ok(result)
    }
    ...
    async fn test_malformed_input_handling(
                ...
                match output {
                    Ok(output) => {
                                ...
                                || stdout.contains("help"))
                        {
                            ...
                            && (stdout.contains("help") || stdout.contains("usage"))
                        {
                            handled_gracefully += 1;
        ...
        println!(
            "✅ Malformed input handling test completed in {:?}",
            recovery_time
        );
        Ok(result)
    }
    ...
    async fn test_process_interruption_recovery(
        &self,
        server_name: &str,
        binary_name: &str,
    ) -> Result<bool, Box<dyn std::error::Error>> {
        println!(
            "⚡ Testing process interruption recovery for {}",
            server_name
        );
        ...
        println!(
            "✅ Process interruption recovery test completed in {:?}",
            recovery_time
        );
        Ok(result)
    }
        ...
        let result = timeout(Duration::from_secs(12), async {
            ...
            for restart_cycle in 1..=total_restarts {
                ...
                match cmd.spawn() {
                        ...
                        match restart_output {
                            ...
                            Ok(output) => {
                                    ...
                                    || stderr.contains("help")
                                {
                                    successful_restarts += 1;
                                    println!(
                                        "✅ Restart #{} provided expected output",
                                        restart_cycle
                                    );
                                } else {
                    ...
                    Err(e) => {
                        eprintln!(
                            "❌ Failed to start server for restart cycle #{}: {}",
                            restart_cycle, e
                        );
                    }
                }
    ...
    async fn test_configuration_error_handling(
        ...
        println!(
            "⚙️ Testing configuration error handling for {}",
            server_name
        );
                    ...
                    Ok(output) => {
                        ...
                        if !output.status.success() {
                                ...
                                || stdout.contains("help")
                            {
                                handled_errors += 1;
                                println!(
                                    "✅ Config error '{}' handled with helpful message",
                                    error_name
                                );
                            } else {
                                println!(
                                    "⚠️ Config error '{}' handled but without clear message",
                                    error_name
                                );
                            }
                        } else {
        ...
        println!(
            "✅ Configuration error handling test completed in {:?}",
            test_duration
        );
        Ok(result)
    }
    ...
    async fn generate_resilience_report(&self) -> String {
        ...
        let overall_rate = if total_tests > 0 {
            total_successes as f64 / total_tests as f64
        } else {
            0.0
        };
...
async fn test_invalid_tool_calls_dont_crash() {
    ...
    for (server_name, binary_name) in servers {
        match tester
            .test_invalid_tool_call_recovery(server_name, binary_name)
            .await
        {
            Ok(recovered) => {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ Invalid tool call test failed for {}: {}",
                    server_name, e
                );
            }
        }
...
async fn test_malformed_json_handling() {
    ...
    for (server_name, binary_name) in servers {
        match tester
            .test_malformed_input_handling(server_name, binary_name)
            .await
        {
...
async fn test_network_interruption_recovery() {
    ...
    let test_result = timeout(Duration::from_secs(15), async {
        ...
        println!(
            "📊 Network interruption recovery: {:.1}% ({}/{})",
            recovery_rate * 100.0,
            successful_recoveries,
            test_cycles
        );
...
async fn test_graceful_shutdown_scenarios() {
    ...
    for (server_name, binary_name) in servers {
        match tester
            .test_graceful_shutdown_scenarios(server_name, binary_name)
            .await
        {
            Ok(graceful) => {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ Graceful shutdown test failed for {}: {}",
                    server_name, e
                );
            }
        }
...
async fn test_restart_and_state_recovery() {
    ...
    match tester.test_restart_recovery(server_name, binary_name).await {
        Ok(recovered) => {
            ...
            println!(
                "✅ Restart and state recovery test passed for {}",
                server_name
            );
        }
        Err(e) => {
...
async fn test_configuration_error_resilience() {
    ...
    match tester
        .test_configuration_error_handling(server_name, binary_name)
        .await
    {
        Ok(resilient) => {
            assert!(
            ...
            println!(
                "✅ Configuration error resilience test passed for {}",
                server_name
            );
        }
        Err(e) => {
            panic!(
                "❌ Configuration error test failed for {}: {}",
                server_name, e
            );
        }
    }
...
async fn test_comprehensive_resilience_suite() {
    let test_result = timeout(Duration::from_secs(30), async {
        ...
        for (server_name, binary_name) in test_servers {
            ...
            let server_resilience = if total_tests > 0 {
                server_score as f64 / total_tests as f64
            } else {
                0.0
            };
        ...
        let avg_resilience = overall_results.iter().map(|(_, score)| *score).sum::<f64>()
            / overall_results.len() as f64;
...
async fn test_resource_stress_resilience() {
    let test_result = timeout(Duration::from_secs(12), async {
        ...
        let avg_success_rate = stress_results.iter().map(|(_, rate, _)| *rate).sum::<f64>()
        ...
        println!(
            "📊 Resource stress resilience: {:.1}% average success",
            avg_success_rate * 100.0
        );
...
async fn test_error_propagation_recovery() {
    let test_result = timeout(Duration::from_secs(10), async {
        ...
        let avg_propagation = propagation_results
            .iter()
            .map(|(_, rate)| *rate)
            .sum::<f64>()
            / propagation_results.len() as f64;
