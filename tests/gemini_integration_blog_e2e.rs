...
async fn test_real_blog_generation_technology() {
    ...
    assert!(
        result.is_ok(),
        "Technology blog generation should not timeout"
    );
    result
        .unwrap()
        .expect("Should generate high-quality technology blog post");
}
...
async fn test_real_blog_generation_business() {
    ...
    assert!(
        result.is_ok(),
        "Business blog generation should not timeout"
    );
    result
        .unwrap()
        .expect("Should generate high-quality business blog post");
}
...
async fn test_real_blog_generation_health() {
    ...
    result
        .unwrap()
        .expect("Should generate high-quality health blog post");
}
...
async fn test_blog_generation_writing_styles() {
    ...
    for style in styles {
        let result = timeout(
            Duration::from_secs(60),
            test_gemini_blog_generation("Remote Work Productivity Tips", style, 600, "general"),
        )
        .await;

        assert!(
            result.is_ok(),
            "Blog generation with {} style should not timeout",
            style
        );
        result.unwrap().expect(&format!(
            "Should generate quality blog post in {} style",
            style
        ));
    }
}
...
async fn test_blog_generation_word_counts() {
    ...
    for word_count in word_counts {
        ...
        assert!(
            result.is_ok(),
            "Blog generation with {} words should not timeout",
            word_count
        );
        result.unwrap().expect(&format!(
            "Should generate quality blog post with {} words",
            word_count
        ));
    }
}
...
async fn test_blog_quality_validation() {
    ...
    result
        .unwrap()
        .expect("Should generate blog that passes quality validation");
}
...
async fn test_gemini_api_error_handling() {
    ...
    let result = timeout(Duration::from_secs(30), test_gemini_error_scenarios()).await;

    assert!(result.is_ok(), "Error handling test should not timeout");
    result
        .unwrap()
        .expect("Should handle API errors gracefully");
}
...
async fn generate_blog_with_gemini(
    ...
    println!(
        "✅ Gemini API returned blog content ({} characters)",
        content.len()
    );
...
/// Validate topic relevance
fn validate_blog_topic_relevance(
    content: &str,
    topic: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if relevance_ratio < 0.3 {
        // At least 30% of topic words should appear
        return Err(format!(
...
/// Validate style consistency
fn validate_blog_style_consistency(
    content: &str,
    style: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match style {
        "professional" => {
            ...
            if content.contains("!!!") || content.split('.').count() < 5 {
                return Err(
                    "Professional style should have formal tone and proper sentences".into(),
                );
            }
        }
        "casual" => {
    ...
    println!(
        "✅ Style consistency validation passed for '{}' style",
        style
    );
    Ok(())
}
...
async fn test_gemini_error_scenarios() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match generate_blog_with_gemini("Technology", "professional", 50000, "general").await {
        Ok(content) => {
            println!(
                "✅ Large word count handled (generated {} chars)",
                content.len()
            );
        }
