...
async fn test_filesystem_server_startup_with_files() {
    ...
    result
        .unwrap()
        .expect("Server should start successfully with test files");
}
...
async fn test_filesystem_server_file_environment() {
    ...
    result
        .unwrap()
        .expect("Server should handle diverse file environment");
}

/// Test filesystem server error handling with invalid arguments
#[tokio::test]
async fn test_filesystem_server_error_handling() {
    let result = timeout(Duration::from_secs(6), test_server_error_scenarios()).await;
...
async fn test_filesystem_server_nested_directories() {
    let result = timeout(Duration::from_secs(8), test_server_with_nested_structure()).await;

    assert!(result.is_ok(), "Nested directories test should not timeout");
    result
        .unwrap()
        .expect("Server should handle nested directory structure");
}

/// Test filesystem server lifecycle - start, run briefly, stop
#[tokio::test]
async fn test_filesystem_server_lifecycle() {
    let result = timeout(Duration::from_secs(12), test_server_startup_and_shutdown()).await;

    assert!(result.is_ok(), "Server lifecycle test should not timeout");
    result
        .unwrap()
        .expect("Server should start and stop cleanly");
}

/// Test server startup with test files
async fn test_server_startup_with_test_files(
        ...
        Some(status) => {
            if status.success() {
                ...
                if stderr.contains("not implemented") || stderr.contains("TODO") {
                    ...
                    return Err(
                        format!("Server failed: stderr={}, stdout={}", stderr, stdout).into(),
                    );
                }
            }
        }
...
async fn test_server_in_diverse_file_environment(
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let temp_dir = TempDir::new()?;
    let base_path = temp_dir.path();
...
async fn test_server_error_scenarios() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
        ...
        if nonexistent_output.status.success() {
            "accepted"
        } else {
            "rejected"
        },
...
fn create_realistic_test_files(base_path: &Path) -> std::io::Result<()> {
    ...
    fs::write(
        ...
        "This is a test file for the filesystem server.\nIt contains multiple lines of text.",
    )?;
    fs::write(
        base_path.join("data.txt"),
        "Sample data content\nLine 2\nLine 3",
    )?;
    fs::write(
        base_path.join("config.txt"),
        "server=filesystem\nport=8080\ndebug=true",
    )?;
    ...
    fs::write(
        ...
        r#"{"name": "filesystem-server", "version": "1.0.0"}"#,
    )?;
    fs::write(
        base_path.join("manifest.json"),
        r#"{"files": ["readme.txt", "data.txt"]}"#,
    )?;
...
fn verify_nested_structure_exists(
    base_path: &Path,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
