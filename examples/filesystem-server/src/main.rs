    ...
    fn resolve_path(&self, requested_path: &str) -> Result<std::path::PathBuf, McpError> {
        ...
        if let Ok(canonical) = full_path.canonicalize() {
            ...
            if let Ok(canonical_parent) = check_path.canonicalize() {
                let intended_canonical = canonical_parent.join(full_path.strip_prefix(&check_path).unwrap_or(&full_path));
...
async fn create_server(base_dir: String) -> Result<McpServerImpl> {
    ...
    info!("Created filesystem server with {} tools", server.tool_count().await);
    Ok(server)
}
...
mod tests {
    ...
    async fn test_read_existing_file() {
        ...
        args.insert("path".to_string(), serde_json::Value::String("test.txt".to_string()));
    ...
    async fn test_read_nonexistent_file() {
        ...
        let mut args = HashMap::new();
        args.insert("path".to_string(), serde_json::Value::String("nonexistent.txt".to_string()));
    ...
    async fn test_path_traversal_protection() {
        ...
        let mut args = HashMap::new();
        args.insert("path".to_string(), serde_json::Value::String("../../../etc/passwd".to_string()));
    ...
    fn test_tool_metadata() {
        ...
        assert!(schema["required"].as_array().unwrap().contains(&serde_json::Value::String("path".to_string())));
    }
}